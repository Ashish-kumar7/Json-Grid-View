...
const NewPreviewPage = () => {
  ...
  const handleCloseGenerated = () => setShowGenerated(false);
  const handleShowGenerated = () => setShowGenerated(true);
  ...
  const handleConversion = (val) => {
    ...
    .post("http://localhost:50000/api/convert", formData)
      .then((response) => {
        console.log("response rec from backend!!");
        console.log(response);
        if(response.data.status == 'success') {
          var genFile = response.data.directory + response.data.filename + response.data.extension;
  ...
  return (
    ...
    <div className="newpreview">
      ...
      >
        ...
        <Modal.Body>
            File generated inside  './Generated Files/ ' directory 
        </Modal.Body>
        <Modal.Footer>
